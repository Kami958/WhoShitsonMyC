...
像 `C:\Windows\WinSxS` 这类地方有很多硬链接，同一个文件可能被算好几次；这里也是逻辑大小，不是按簇算的物理占用。两次扫描之间的**差值**仍然靠谱
